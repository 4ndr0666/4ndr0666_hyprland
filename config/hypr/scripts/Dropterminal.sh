...
calculate_dropdown_position() {
	...
	if command -v bc >/dev/null 2>&1; then
		...
		logical_width=$(echo "scale=0; $mon_width / $mon_scale" | bc)
		logical_height=$(echo "scale=0; $mon_height / $mon_scale" | bc)
		# Strip any residual decimal bc may emit despite scale=0
		logical_width=${logical_width%%.*}
		logical_height=${logical_height%%.*}
	else
		# Fallback to integer math.
		# Convert scale to centesimal integer robustly:
		#   "1"   → 100,  "1.5" → 150,  "2.0" → 200
		# The original sed 's/\.//' path breaks on bare integers like "1" → "1"
		# (not "100"), producing wildly wrong logical dimensions. Fixed here.
		local scale_int
		if [[ "$mon_scale" == *.* ]]; then
			...
			scale_int=$(( (int_part * 100) + 10#$frac_part ))
		else
			# Integer scale value — multiply by 100
			scale_int=$(( mon_scale * 100 ))
		fi
		[ "$scale_int" -eq 0 ] && scale_int=100
...
spawn_terminal() {
	...
	# Get window list before spawning
	...
	# Rule syntax: "workspace special:scratchpad silent" is a single token —
	# preserved exactly as the original; splitting 'silent' broke on some builds.
	...
	if [ "$count_after" -gt "$count_before" ]; then
		# Find the new window by comparing before/after address lists
		new_addr=$(comm -13 \
	...
	# Fallback: most recently mapped window by focusHistoryID
	...
	if [ -n "$new_addr" ] && [ "$new_addr" != "null" ]; then
		...
		# Bring to current workspace silently (no workspace history pollution)
...
if terminal_exists; then
	...
	if terminal_in_special; then
		...
		# Move to current workspace silently, pin, resize, then animate
		...
		if [ -n "$geometry" ]; then
			...
			# Animate slide up first, then move to scratchpad and unpin (toggle)
