...
# === 4ndr0666 === #
# Purpose:
#   Orchestrates copying/upgrading 4ndr0666's Hyprland dotfiles into ~/.config.
...
echo "${MAGENTA}Kindly visit 4ndr0666 Hyprland Own Wiki for changelogs${RESET}"
printf "\n%.0s" {1..1}
...
if [ "$EXPRESS_MODE" -eq 1 ]; then
  ...
  while true; do
    ...
    [Yy])
      ...
      if git clone "https://github.com/4ndr0666/Wallpaper-Bank.git"; then
...
printf "\n%.0s" {1..2}
printf "${OK} GREAT! 4ndr0666's Hyprland-Dots is now Loaded & Ready !!! "
printf "\n%.0s" {1..1}
printf "${INFO} However, it is ${MAGENTA}HIGHLY SUGGESTED${RESET} to logout and re-login or better reboot to avoid any issues"
printf "\n%.0s" {1..1}
printf "${SKY_BLUE}Thank you${RESET} for using ${MAGENTA}4ndr0666's Hyprland Configuration${RESET}... ${YELLOW}ENJOY!!!${RESET}"
printf "\n%.0s" {1..3}