...
configure_waybar_links() {
  ...
  if [[ -e "$WAYBAR_CONFIG" && ! -L "$WAYBAR_CONFIG" || -e "$HOME/.config/waybar/style.css" && ! -L "$HOME/.config/waybar/style.css" ]]; then
    printf '%s' '[ACTION] Replace existing Waybar config/style files with managed symlinks and remove obsolete layouts? [y/N] ' >/dev/tty
    read -r answer </dev/tty
    case "$answer" in
      y|Y|yes|YES) ;;
      *)
        printf '%s\n' '[INFO] Existing Waybar config/style ownership retained; skipping link transaction.' | tee -a "$LOG"
        return 0
        ;;
    esac
  fi
