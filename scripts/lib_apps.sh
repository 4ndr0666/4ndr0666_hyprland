...
enable_asusctl() {
  ...
  if command -v asusctl >/dev/null 2>&1; then
    ...
    touch "$OVERLAY_SA"
    grep -qx 'exec-once = rog-control-center' "$OVERLAY_SA" \vert{}\vert{} echo 'exec-once = rog-control-center' >>"$OVERLAY_SA"
  fi
}
...
enable_blueman() {
  ...
  if command -v blueman-applet >/dev/null 2>&1; then
    ...
    mkdir -p "$(dirname "$OVERLAY_SA")"
    touch "$OVERLAY_SA"
    grep -qx 'exec-once = blueman-applet' "$OVERLAY_SA" \vert{}\vert{} echo 'exec-once = blueman-applet' >>"$OVERLAY_SA"
  fi
}
...
enable_ags() {
  ...
  if command -v ags >/dev/null 2>&1; then
    ...
    mkdir -p "$(dirname "$OVERLAY_SA")"
    touch "$OVERLAY_SA"
    grep -qx 'exec-once = ags' "$OVERLAY_SA" \vert{}\vert{} echo 'exec-once = ags' >>"$OVERLAY_SA"
...
enable_quickshell() {
  ...
  if command -v qs >/dev/null 2>&1; then
    ...
    mkdir -p "$(dirname "$OVERLAY_SA")"
    touch "$OVERLAY_SA"
    grep -qx 'exec-once = qs' "$OVERLAY_SA" \vert{}\vert{} echo 'exec-once = qs' >>"$OVERLAY_SA"
...
install_terminal_configs() {
  ...
  if [ -f "$GHOSTTY_SRC" ]; then
    mkdir -p "$GHOSTTY_DIR"
    install -m 0644 "$GHOSTTY_SRC" "$GHOSTTY_DEST" 2>&1 \vert{} tee -a "$log"
    if [ -f "$GHOSTTY_DIR/wallust.conf" ]; then
      sed -i -E 's/^(\\s*palette\\s*=\\s*)([0-9]{1,2}):/\\1\\2=/' "$GHOSTTY_DIR/wallust.conf" 2>&1 \vert{} tee -a "$log" || true
    fi
  else
  ...
  if [ -f "$WEZTERM_SRC" ]; then
    mkdir -p "$WEZTERM_DIR"
    install -m 0644 "$WEZTERM_SRC" "$WEZTERM_DEST" 2>&1 \vert{} tee -a "$log"
  else
...
choose_default_editor() {
  ...
  update_editor() {
    ...
    echo "${OK:-[OK]} Default editor set to ${MAGENTA:-}$editor${RESET:-}." 2>&1 \vert{} tee -a "$log"
  }
  if command -v nvim &>/dev/null; then
    ...
    if ! read -r -p "${CAT:-[ACTION]} Do you want to make ${MAGENTA:-}neovim${RESET:-} the default editor? (y/N): " EDITOR_CHOICE </dev/tty; then
      :
    elif [[ "$EDITOR_CHOICE" == "y" \vert{}\vert{} "$EDITOR_CHOICE" == "Y" ]]; then
      update_editor "nvim"
      editor_set=1
    fi
  fi
  ...
  if [[ "$editor_set" -eq 0 ]] && command -v vim &>/dev/null; then
    ...
    if read -r -p "${CAT:-[ACTION]} Do you want to make ${MAGENTA:-}vim${RESET:-} the default editor? (y/N): " EDITOR_CHOICE </dev/tty; then
      if [[ "$EDITOR_CHOICE" == "y" \vert{}\vert{} "$EDITOR_CHOICE" == "Y" ]]; then
        update_editor "vim"
        editor_set=1
      fi
    fi
