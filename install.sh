...
whiptail --title "4ndr0666 Arch-Hyprland (2025) Install Script" \
    --msgbox "Welcome to 4ndr0666 Arch-Hyprland (2025) Install Script!!!\n\n\
...
echo "👌 ${OK} ${MAGENTA}4ndr0666..${RESET} ${SKY_BLUE}lets continue with the installation...${RESET}" | tee -a "$LOG"
...
options_command+=(
    ...
    "dots" "Download and install pre-configured 4ndr0666 Hyprland dotfiles?" "OFF"
)
...
while true; do
    ...
    if [[ "$dots_selected" == "OFF" ]]; then
        ...
        if ! whiptail --title "4ndr0666 Hyprland Dot Files" --yesno \
        "You have not selected to install the pre-configured 4ndr0666 Hyprland dotfiles.\n\nKindly NOTE that if you proceed without Dots, Hyprland will start with default vanilla Hyprland configuration and I won't be able to give you support.\n\nWould you like to continue install without 4ndr0666 Hyprland Dots or return to choices/options?" \
    ...
    echo "👌 ${OK} You confirmed your choices. Proceeding with ${SKY_BLUE}4ndr0666 Hyprland Installation...${RESET}" | tee -a "$LOG"
    break  
done
...
echo "${INFO} Installing ${SKY_BLUE}4ndr0666 Hyprland additional packages...${RESET}" | tee -a "$LOG"
sleep 1
...
for option in "${options[@]}"; do
    case "$option" in
        ...
        rog)
            ...
            ;;
        dots)
            echo "${INFO} Installing pre-configured ${SKY_BLUE}4ndr0666 Hyprland dotfiles...${RESET}" | tee -a "$LOG"
...
if pacman -Q hyprland &> /dev/null || pacman -Q hyprland-git &> /dev/null; then
    ...
    printf "${SKY_BLUE}Thank you${RESET} 🫰 for using ${MAGENTA}4ndr0666's Hyprland Dots${RESET}. ${YELLOW}Enjoy and Have a good day!${RESET}"
    printf "\n%.0s" {1..2}
