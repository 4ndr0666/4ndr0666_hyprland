...
if git grep -nE 'JaKooLit/Hyprland-Dots|KooL_Dots_DIR|Hyprland-Dots\.git' -- . \
  ':!config/hypr/v2.3.20' \
  ':!tests/unit/test-config-only-dot-repository.sh' >/dev/null; then
  printf '%s\n' 'external dot repository implementation reference remains' >&2
  exit 1
fi
