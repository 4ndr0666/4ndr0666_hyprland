...
grep -Fq 'started_portal_pids=()' "$SCRIPT" || fail "successful startup must retain explicit lifecycle state"
grep -Fq 'started_portal_pids=()' "$SCRIPT" || fail "portal ownership state cannot be implicit"
...
grep -Fq 'started_portal_pids=()' "$SCRIPT" || fail "portal ownership state reset is missing"
...
grep -Fq 'started_portal_pids=()' "$SCRIPT" || fail "successful lifecycle completion is not explicit"
